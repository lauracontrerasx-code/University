...
function subjectsPage() { return `<section class="stack"><div class="page-heading"><div><h2>Subjects</h2><p>Courses, topics, and grading.</p></div></div><div class="list">${state.subjects.map(subjectCard).join("") || empty("No subjects yet. Add your first subject with the + button.")}</div></section><button class="fab" data-action="add-subject" aria-label="Add subject">+</button>`; }
...
function bindPageEvents() { const app = document.querySelector("#app"); app.querySelectorAll("[data-route]").forEach(button => button.addEventListener("click", () => navigate(button.dataset.route))); app.querySelectorAll("[data-action]").forEach(button => button.addEventListener("click", () => action(button.dataset.action, button.dataset.id))); app.querySelectorAll("form[data-form]").forEach(form => form.addEventListener("submit", submitForm)); }
